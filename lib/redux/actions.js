...
export default {
  ...
  run(config, dispatch) {
    let child = fork(__dirname + '/../process/webpack.js', [], {
    ...
    child.stdout.on('data', (data) => console.log(`[Webpack Child ${child.pid}] ${data}`));
    child.stderr.on('data', (data) => console.log(`[Webpack Child ${child.pid}] ${data}`));
    console.log(`Child process with pid ${child.pid}`);
    ...
    child.on('exit', () => {
      console.log(`Webpack, pid ${child.pid} has exited`);
    });
  },
